...
For my second project as part of the Metis Data Science Bootcamp, I web-scraped NBA player statistics from [Basketball-Reference](https://www.basketball-reference.com) to build a linear regression model to predict their salaries.
...
- "Stocks". This is steals + blocks, which is commonly used as an aggregate term in NBA analytics circles to simplify defensive stats
...
- [True Shooting Percentage](https://www.breakthroughbasketball.com/stats/tsp_calc.html) (TS%). True Shooting Percentage is commonly used in the NBA as a metric to judge overall shooting. It takes into account field goals, three pointers, and free throws into one comprehensive number. The formula is Points / (2 * (FGA + 0.44 * FTA))
...
- Turnovers are positively correlated with salary. While counterintuitive, this makes sense because better players get more playing time and run the offense more frequently, leading to more turnovers. To control for this, I analyzed two commonly used metrics: assist-to-turnover ratio and turnover rate (or turnovers per 100 possessions estimate). Even after controlling for player skill and playing time, positive correlation still existed, suggesting that teams don’t typically punish turnovers. I kept TOV% out of the model for this reason.
- Although I experienced multicollinearity issues with points, I kept it in due to the obvious significance of this statistic. Excluding this may lose credibility with potential stakeholders. This same logic applies to year and age.
...
- In total, there are 1381 records of players from 2018-2021 being used for this analysis
